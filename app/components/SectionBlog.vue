<template>
  <div class="min-h-[70vh] container px-2 lg:px-0 mx-auto mt-16 mb-4" v-if="posts && posts.length > 0 && isBlogActive">
...
const { data: posts } = await useAsyncData(`blog-posts:limit-${postsLimit.value}:locale-${locale.value}`, () => queryCollection(`esPosts`)
